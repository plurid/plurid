...
DOCKING AND THE PAGE PRESENTATION (2026-09-05): `space.presentation: 'page'` makes the space present as a SITE. It is ONE knob with three defaults layered under the caller's configuration in `merge()` (`pagePresentationDefaults`: `fadeInTime: 0`, `opaque: false`, `elements.plane.height: 1`) and ONE derived state, the docked plane; there is no mode flag — the pose of the camera is the state. `elements.plane.height` (flat `planeHeight`; ≤ 1 a fraction of the view height, > 1 px) is the general mechanism: the height every UNDECLARED plane renders at (`space/layout/size.ts` : `configuredPlaneSize`; the Plane renders it, the layouts pitch their rows by it), so in the page presentation every plane is view-sized. The camera DOCKS on a page (`interaction/camera/dock.ts`): `dockPose` = yaw −rotateY, pitch −rotateX, scale 1, pivot the plane center, offset 0 — with `M = T(C+offset)·Rx·Ry·S·T(−pivot)` a view-sized plane then maps to view px 1:1 (depth 0, perspective factor 1), so at boot the identity camera IS the dock pose of root 0 and no fit runs. `isDocked` compares the PICTURE, not the parameters (zoom-at-cursor and the cursor pivot re-parameterize pivot/offset losslessly): scalar early exits (scale, yaw, pitch within 1e-3), then the projected top-left and bottom-right corners within half a pixel of (0,0) and (W,H). `dockGeometry` resolves the box a plane docks by: a declared or manual size is the plane's own; a MEASURED size is only an observation of the configured one (it lags a frame behind a view resize and starts at 0), so a configured dimension wins over it — without this the boot and every resize flashed the chrome for one frame (the `page.spec.ts` boot scenario records every frame). `findDockedPlane` walks the shown tree; `dockCandidate` picks the shown plane whose projected center is nearest the view center; `revealPose` is the dock pose pulled back to scale 0.8 and tilted (pitch 8, yaw −6: `REVEAL`). The react side reads the presentation in exactly four places: `frameTargetForPlane` returns the dock pose instead of a fitted frame — so EVERY framing path docks (the frame control, a link spawn, `navigateToParent`, `onClose: 'parent'`, the arrows, the minimap, `space.frame`) while `fit` still fits, a legitimate undock; the memoized selector `getDockedPlaneID` (camera + tree + view; `''` outside the presentation, so an undocked orbit re-renders nothing); the Plane (the controls bar hangs ABOVE the sheet, `top: -56px`, instead of taking a grid row, so the docked page is whole and never jumps; the content scroller gets `tabIndex -1` and the focus when its page docks, so PageDown / Space / arrows scroll it); the Viewcube (its bottom band becomes a RAIL of round 32 px buttons with a 16 px margin — fit, a `dock-back` chevron when the docked page has a parent, and the corner control `data-plurid-control="dock-toggle"`; the rail's dock buttons are the only chrome that stays while docked, as translucent dark pills with a light rim so they read on any page). The selector surfaces as ONE DOM attribute, `data-plurid-docked="<planeID>"` on the View: CSS does every chrome fade (`chromeDocked` in `services/styled/chrome.ts` on the toolbar, viewcube, minimap, origin, shortcuts trigger and plane controls — instant on dock, because a site must never flash its 3D chrome, 240 ms in on the reveal), and the wheel rule (`WheelContext.docked` → `scroll`: a plain wheel over a docked page is the page's, it scrolls or does nothing; pinch, Ctrl, Shift, Alt and grab still move the camera), Escape (`dock`, before `exitGrabMode`), G (`reveal` when entering grab while docked), the corner control and the resize re-dock (`animate: false` after the relayout) read the same selector. `CameraCommand` gains `dock` (that plane, else the docked one, else the candidate) and `reveal`; topics `space.dock` `{ planeID?, animate? }` / `space.reveal` `{ animate? }`. The wheel's smoothed tail is dropped when a page docks and never fights a tween (`wheelBatcher.cancel()` on the dock transition; the batcher skips frames while `motion === 'tween'`). DOCKING CONTROLS (`space.docking`, 2026-09-06): `commitCameraTarget` — the one exit of every programmatic move — learns whether the target LANDS DOCKED (`landingDockPlaneID`: the page presentation and `findDockedPlane` on the target); with `docking.motion: 'instant'` such a move jumps whatever the caller asked (a link, back, `onClose: 'parent'`, Escape, `space.dock`; the reveal never lands docked, so it keeps its motion — `navigatePlane`'s focus timer is 0 then too), and a docking tween records its destination in `state.space.dockingPlaneID` (written after `tweenTo`, which first stops any running tween and resets the motion; cleared by `setMotion` whenever the motion leaves `tween`). `getDockedPlaneID` falls back to that destination while `motion === 'tween'` and `docking.chrome` is `hidden` (the default): the destination page counts as docked for the whole swing, so `data-plurid-docked` never drops and the chrome never paints between pages (a 240 ms fade-in inside a 380 ms tween was a visible flicker); `chrome: 'shown'` restores the space during the swing. The docked STATE stays derived; what is stored is the tween's destination, the one fact about a tween the camera cannot tell yet. THE LINEAGE (2026-09-06): while a page is docked (or a swing is docking) only its lineage is shown — `getDockedLineage` (a memoized `Set` from the docked id and the tree: `computePath` ancestors + the page + `collectPlaneIDs` descendants) and the per-plane `makeGetIsPlaneAside`; a plane outside it carries `data-plurid-aside` and fades through the inline opacity it already owns (240 ms both ways, none under reduced motion; `pointer-events: none`). Two links in one header spawn two parallel view-sized pages a few dozen pixels apart, and the one opened last would otherwise cover the one clicked; a root-to-root dock fades the source root from the first frame (roots are not each other's lineage). SSR parity: the server renders the identity camera, so the HTML is the docked page with the chrome hidden. Verified by `camera/__tests__/dock.test.ts`, the react command / selector / Plane tests, `fixtures/render-test/e2e/page.spec.ts` (nine scenarios: boot without a frame of chrome, wheel scrolls / does nothing on a short page, Ctrl + wheel then Escape, the corner control, a link docks onto the child and back, G / Escape, one-finger touch scroll, a resize) and four visual baselines (`page-docked`, `page-revealed`, `page-spawned`, `pages-3-revealed`).
...
The page presentation in the harness (2026-09-05): `?presentation=page&pages=N` (the SITE set: N root pages with an about and a contact sub-page, `src/Site.tsx`), fixtures `page-docked` / `page-revealed` / `page-spawned` / `pages-3-revealed` (the `revealed` viewpoint publishes `space.reveal`), `e2e/page.spec.ts`, and the links invariant of `fixtures.spec.ts` treats chrome over a link (the minimap over a revealed page's header) as legitimate.
