...
`TreePlane` essentials (`plurid-data source/interfaces/internal/tree` : `TreePlane`): `sourceID` (the registered plane it instantiates), `planeID` (the runtime instance id), `parentPlaneID`, `route`, measured `width`/`height` (+ `sizeMode`), `location` (`translateX/Y/Z`, `rotateX/Y`), `show`, `children`, `bridgeLength`/`planeAngle`/`bridgeSide`/`linkCoordinates`/`spawnedByLinkID` (spawn geometry — the edge the bridge leaves from included — and the identity of the link that spawned it), and `manuallyPositioned` - set when the user drags a plane; auto-layout then leaves it pinned (its location carries across relayouts) and arranges only the un-pinned planes.
...
| `?hostileCss=1` | a host stylesheet with aggressive global resets (form-control sizing, typography) — the chrome must keep its sizes (`chrome.spec.ts`) |
| `?scrollable=1` | the GEOMETRY readout is a scroller (28 filler rows in a 120px box): a wheel over it only scrolls it, even at the end of its range, while the rest of the plane zooms (`rendering.spec.ts`) |
