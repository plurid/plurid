...
- **A measurement is an observation.** The Plane's ResizeObserver reports `offsetWidth/Height` rounded to 0.5 px through the equality-gated `setPlaneSize`; a `manual` plane ignores reports; a culled plane does not report; nothing measured is ever persisted as intent (`sizeMode` records whose the size is).
...
Known limit: a dragged child keeps its bridge stub where the link geometry would put it; the leash follows a scrolled LINK, not a moved child (a follow-up).
