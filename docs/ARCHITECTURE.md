...
- which read it via `useContext` - do not re-render on every per-frame View render. The behavior lives in the hooks of `containers/Application/View/hooks/`:
...
| `useTreeUpdate` | `treeUpdate`/`treeUpdateCallback`/`resolveLayout`: rebuild the roots from the view + registered planes (THE SIZING CONTRACT: placed by the previous tree's content-driven sizes), carry each root's runtime state across by IDENTITY (`pairRootsByIdentity`: source + route, in order for duplicates — never by location, a relayout exists to move things), keep the duplicates of a listed identity, re-place every spawned subtree | `hooks/useTreeUpdate.ts` |
...
| `useCulling` | the culling pass, at most every 100 ms after a camera commit or a tree change: hidden / frozen planes, the detach tier (`resolveDetached`, a `hiddenSince` clock and a maturity timer), the depth cues as CSS variables on the plane elements | `hooks/useCulling.ts` |
| `useMeasuredRelayout` | THE SIZING CONTRACT's relayout: a root's measured height changes → one relayout per frame, gliding after the boot's first, deferred while the camera moves or a transition runs | `hooks/useMeasuredRelayout.ts` |
| `useGamepad` | opt-in gamepad navigation (`gestures.gamepad.enabled`): the sticks orbit and pan, the triggers zoom, one loop while a pad is held | `hooks/useGamepad.ts` |
| `useDockingURL` | THE ADDRESS BAR IS THE PAGE (`space.docking.url`): writes the docked page's path (one entry per page, the first write replaces the entry, the reveal keeps it), restores a deep link on mount (a root is docked at STORE time — `state.compute` lays the roots out and seeds the camera through `dockedBootCamera`, the client and the server alike; a registered sub-page is spawned through its parent's `PluridLink`, one level per commit; a path no registrar answers to settles at once under the `orphan` rule; a `base` prefix is read from under and written after), docks on `popstate`; declared after the relayout and follow effects so a restore docks with the roots measured; the query mode (`?page=`) inside a `PluridRouterBrowser` route (`PluridRouterContext`) | `hooks/useDockingURL.ts` |
...
```
CameraState = { yaw, pitch, roll, scale, pivot: Vec3, offset: Vec3, perspective }
M = T(C + offset) · Rz(roll) · Rx(pitch) · Ry(yaw) · S(scale) · T(−pivot)     C = view center
```
...
- The rotation is the explicit CSS composition `Rz(roll) · Rx(pitch) · Ry(yaw)` (`rotationXMatrix`/`rotationYMatrix`/`rotationZMatrix` are pinned to the CSS `rotateX`/`rotateY`/`rotateZ` definitions by tests); no quaternion on the camera path.
- THE HORIZON (`roll`, 2026-09-13) is the third angle, applied FIRST in camera space, which is what makes it a tilt of the PICTURE rather than a rotation of the world: it moves no plane on screen but the angle they are all seen at. It wraps like the yaw (`clampCamera`, `navigation.rollLimit` holds it near level, `0` forbids it), a tween takes the short way round it (`interpolateCamera`), and `setPivot` re-parameterizes through the full rotation so a rolled camera re-pivots losslessly. IT IS 0 IN EVERY FRAMING THE ENGINE COMPUTES — `dockPose`, `revealPose`, `frameBounds` (so frame / fit / home / presets / the viewcube faces), `fromLegacy` — and `isDocked` requires a level horizon, which keeps the ordinary space a turntable, makes `0` (fit) the way back to level, and means no fixture picture can drift. Only first person (`Z` / `C`, `useFlyControls`) and a host's own `CameraDelta` tilt it. The viewcube composes it (the one cue for which way is up while flying); the minimap deliberately does not (it maps the world, not the picture). The legacy six scalars cannot hold a third angle: `toLegacy` un-rotates the offset by the FULL rotation (exact) and drops the roll from `rotationX/Y` (documented, lossy, and nothing reads the scalars back to move the camera).
...
CULLING (`useCulling`, at most one pass per 100 ms after a camera commit or a tree change; `plurid-engine modules/space/view/culling.ts`): `cullPlanes` decides which shown planes stop painting — every projected corner outside the view expanded by `space.culling.frustumMargin`, or the center farther from the EYE than `space.culling.distance` — and which are frozen (farther than `freezeDistance`, still painted, contained); both thresholds carry hysteresis against the previous result, and the active, selected, isolated and focused planes and the docked page's lineage are never culled. The result is `state.space.culled` (written only when it changes; never persisted or broadcast — `computeSpace` normalises it after the state spreads). Distances are camera-space from the eye, so a plane at the pivot depth sits at `perspective` (2000) — defaults `freezeDistance` 3500, `distance` 6000; off unless `space.culling.enabled`.

A hidden plane keeps its React state and gets `data-plurid-culled="hidden"` (`visibility: hidden`, `pointer-events: none`, `contain: layout paint style`), a frozen one `data-plurid-culled="frozen"` (`contain: layout paint style`, its `ResizeObserver` stops reporting); `usePluridPlane()` exposes `culled` / `frozen` so a plane can pause video or polling while unseen. With `elements.plane.depthFade` the same pass writes `--plurid-plane-depth` / `--plurid-plane-fade` / `--plurid-plane-blur` on each plane element (no store churn) and the plane's stylesheet fades/blurs with them; `elements.plane.backface: 'hidden'` stops painting planes seen from behind.

THE DETACH TIER (`space.culling.detach`): after `cullPlanes`, the engine's `resolveDetached` decides which HIDDEN planes lose their content — hidden continuously for `delay` ms (the hook keeps a `hiddenSince` clock and a maturity timer that re-runs the pass when the earliest candidate is due), farther than `distance`, not an exception, with every shown child hidden too; `max` bounds the hidden-but-mounted pool (exceptions and parents of shown planes count toward it), trimmed from its farthest detachable planes; nothing new detaches while the camera moves or a layout transition runs, re-attaching is never gated; `culled.detached` is emitted in tree order. The Plane renders a detached plane as a SHELL at the tree's size — `retain` wraps the interior (the bar, the content) in React's `Activity` (`hidden`: state kept, effects unmounted, DOM `display: none`; a React without it falls back to `unmount` with a development warning), `unmount` renders nothing inside but the focus anchor and the bridge — with `data-plurid-culled="detached"` and the content scroller's position remembered on the shell and restored on re-attach.

THE DIAGNOSTIC SURFACE: `api.inspect()` builds one plain `PluridInspection` from the store plus the application's `PluridInspectorRegistry` (`services/logic/inspector`: `createInspector` / `recordRender` / `cullingCounts` / `buildInspection`) — the registry lives on the engine context; every `PluridPlane` counts its render in an effect after the commit (a Map write only while the registry is on: `development.inspector`, or either debugger), the Application's store subscription counts dispatches, `usePointerGestures` reports its intent (`onIntent`) at the press and `null` at the end; `cullingCounts` is the one reading of the tiers for `inspect()`, `handle.tree.culling()` and the `culling` event. The debuggers are `React.lazy` behind their flags and never render on the server; the HUD reads the registry's deltas per half second, the `renderDebugger` slot receives the inspection.

BENCHMARK (`?planes=N&bench=1&benchScenario=orbit|relayout|spawn`, `fixtures/render-test/e2e/bench.spec.ts`, `pnpm bench` for the table): a scripted run of 240 frames — the camera (orbit + pan + zoom, one delta per frame), the view size toggled every 20 frames (relayouts), or the GEOMETRY detail link opened and closed every 30 frames (spawns) — one independent test per scenario and size (the orbit at 40 / 100 / 500 planes, the others at 40 / 100; a larger orbit measures its own 40-plane baseline first, so the scaling assertion compares two runs of the same machine in the same state); the harness reports `__rtBench = { bootMs, firstFrameMs, p50FrameMs, p95FrameMs, maxFrameMs, dispatches, frames }`. ALWAYS asserted: one camera commit per frame (dispatches ≤ frames + 12), no stall (the P95 frame ≤ ten times the run's own median — 2026-09-13: it used to read the single worst frame, which is legitimately the mount frame or a frame the OS took away, and that is how CI #53 failed; the single worst frame is now held only to a separate FREEZE ceiling, high enough that only a real lock-up reaches it), the relayout run's dispatches bounded (one view-size write, one relayout, one size batch, one measured relayout per toggle), and sub-linear scaling relative to the 40-plane p50 measured in the same run (100 planes ≤ 3×, 500 planes ≤ 8×) — the properties a regression breaks on any machine. ABSOLUTE p95 budgets (40 → 40 ms, 100 → 50 ms, 500 → 100 ms) are a property of the machine (a CI runner is several times slower than a development laptop) and are enforced only with `BENCH_STRICT=1`; the numbers are always attached to the report. Measured 2026-09-03 in headless Chromium on the development machine: 40 planes p95 17 ms (p50 8.4), 100 planes p95 17.5 ms (p50 8.4), 500 planes p95 33 ms (p50 16.7); the GitHub runner measured ~100 ms p95 at 500 planes.
...
The address bar (2026-09-06): the docked page's path is the pathname while `docking.url` is on (the page presentation's default); the two URL bindings share one URL — the path is the page's, `?v=` the camera's (`replaceState` keeps the other) — and a deep link's path wins over the persisted camera (the persisted tree stays); the resolver `general.configuration.resolveDockingURL` turns the knob into `{ write, restore, history, mode, param, base, orphan }`, the engine's `planeAddressPath` reduces a plane address to its pathname and `routing.dockingURLTarget` reads a location (the browser's or a request's) under the binding. A ROOT deep link docks at store time (2026-09-10): `resolveSpace` lays the roots out for the boot view when a path is given (the layout-less boot tree stacks every root at the origin, where every dock pose is the first root's) and seeds the camera with `dockedBootCamera`; the View's follow effect carries it through the first measurement. The server does the same through `computeApplication` (the query mode: the application sits in the static router), so the HTML is docked on the deep link's page.
...
### 5.3.1 The sizing contract (2026-09-10)

Who decides a plane's box, and what follows from a change — one rule, so content layout and spatial layout never feed back into each other:

- **Width is never derived from content.** A plane's width is the configured `elements.plane.width`, a declared `planes[].width`, or a hand-set (`manual`) one. A content component that reflows on width therefore changes only its HEIGHT.
- **Height is the content's unless declared, configured (`elements.plane.height`) or manual.** A content-sized height may be CAPPED (`elements.plane.maxHeight`, flat `planeMaxHeight`; a registered plane's own `maxHeight`): taller content scrolls inside the cap and the measured height is the cap (the plane's content scroller takes the keyboard like a declared height's).
- **A measurement is an observation, and a frame's measurements land together.** The application's ONE ResizeObserver (`observePlaneSize`, `services/logic/size`) watches every plane; the browser delivers a frame's observations to its one callback, so the N planes that re-measure after a relayout are one `setPlaneSizes` — one path-copy per changed plane, one `recomputeTree`, one store notification (the resize burst the bench measured at ~110 dispatches per relayout of 100 planes is gone); the untransformed `offsetWidth/Height` are rounded to 0.5 px and the reducer is equality-gated; a `manual` plane ignores reports; a culled plane does not report; nothing measured is ever persisted as intent (`sizeMode` records whose the size is).
- **The roots are placed by their CURRENT sizes.** `Tree.compute` takes the tree being replaced (`previousTree`) and `applyKnownSizes` copies each root's measured or hand-set size onto the fresh root (by identity, in order for duplicates; a declared dimension is never overridden) BEFORE the layout runs — `columns`, `rows`, `faceToFace` and `zigZag` pitch their rows and columns by the tallest and widest plane in each; `sheaves` ignores sizes by design.
- **A measured height change relays the roots** (`useMeasuredRelayout`): once per frame, gliding after the boot's first measured relayout (the fallback height at boot was a guess, so that one is instant), never during a gesture, a fling, a tween or a running layout transition (deferred to the next idle commit), never when a configured height makes every root the same, never for `sheaves`; a pinned (`manuallyPositioned`) or hand-sized root does not trigger it. A relayout changes transforms only, so no measurement follows it: the loop is closed by construction.
- **Every history step has a derived name.** `services/logic/arrangement/describe.ts` reads the two arrangements (the same `arrangementEntries` the rebase uses) and names the change — one plane by its path, several by their count, a mixed change by what it touched, the link graph on its own. The middleware stores `{ snapshot, label, at }` per step and publishes the labels with the status (`past` / `future`); `space/historyGoTo` walks N steps and restores ONCE.

- **A pin is respected everywhere.** `recomputeSubtree` leaves a `manuallyPositioned` child where the user dropped it (its own descendants still follow it); the reconcile carries a pinned root's location; `reconcileTree` compares the spawn geometry (`linkCoordinates` by value, `bridgeSide`, `bridgeOffset`, `spawnedByLinkID`), so a host `setTree` with an unchanged route keeps it.

- **A moved child keeps its link on a leash.** A `manuallyPositioned` child draws no bridge band (a stub `bridgeLength` px off its own edge would point nowhere); the beams layer (`PlaneLinks`, `collectLeashes`) draws a LEASH instead — the strip as a beam from the link's point on the parent's face (`linkWorldPoint`, the point `childLocation` hangs from) to the middle of the child's bridge-side edge on the link's line (`childLeashPoint`), in the roots' frame, so it rides the camera with no per-frame JS and follows every drag per commit; undoing the move brings the band back. `elements.planeBridge.show` hides both.

### 5.4 Spawn, close, links, selection

- SPAWN: `PluridLink` (`components/links/Link`) - the in-space anchor. THE TREE IS THE SINGLE SOURCE OF TRUTH for a link's state: a link has a stable identity (`linkID` prop, else `<parentPlaneID>#<route>#<ordinal>`, the ordinal being its index among the links to the same route inside the plane, DOM order), the plane it spawns records it as `TreePlane.spawnedByLinkID`, and the link's open/closed state and target plane are DERIVED from the tree (`space.tree.fields.findPlaneByLinkID`) - never held in component state, so undo, a host `setTree`, a relayout or a collaboration apply keep every link in step. A click dispatches the `toggleLinkPlane` thunk (`services/state/thunks/planes.ts`): absent -> `updateTreeWithNewPlane` (the spawned plane id is `<route>@<digest of the link id>`, deterministic and uniqued against the tree) then navigate; present -> `togglePlaneFromTree`. Geometry is ONE function, `space.location.childLocation(parentLocation, linkCoordinates, bridgeLength, planeAngle, bridgeSide, childWidth, bridgeOffset)`: the child sits at the end of a bridge from the link point — its TOP (the top of its controls bar) half a strip above the link's midline, so the bridge is centred on the link's line and flush with the plane's top (`TreePlane.bridgeOffset`, `resolveBridgeOffset`: `−BRIDGE_STRIP_HEIGHT / 2`, plus `PLANE_BAR_HEIGHT` on a page where the bar hangs above the sheet; stored with the spawn geometry, 2026-09-06) — turned by `planeAngle`, applied the SAME way every generation by default (`space.bridge.fan: 'fixed'`: each child turns 90° to the right of its parent and hangs behind its parent's face, exactly like the first link off a root — the user's rule, 2026-09-05; `'alternate'` flips the sign by generation so a grandchild faces the way its grandparent does), and whose FIRST sign is the growth direction (`space.bridge.direction: 'backward'` (default) | `'forward'`): backward starts POSITIVE — CSS `rotateY(θ)` turns local +X toward −Z for a positive θ — so a chain grows DEEPER, behind the wall its root sits on (the space is explored by going in; in a wall layout a grandchild can therefore sit behind the neighbouring roots and be seen, and clicked, only through the gaps between them until the camera goes around or in — Chrome paints and hit-tests exactly that geometry, verified 2026-09-05); forward starts negative, the chain grows out of the wall toward the viewer. a generation hangs off its parent's LEFT edge on the side the fanned angle turns to — a grandchild sits on the side its parent fin faces, like the original engine (the user's choice, 2026-09-05). With `space.bridge.keepBehind` the generations that would hang on the side their parent faces are MIRRORED to the other side of the link — `TreePlane.bridgeSide: 'end'` (`resolveBridgeSide`), the bridge leaves the child's right edge and the child's origin sits `bridgeLength + width` back along its own axis (placed with the spawn's `fallbackWidth` until measured; `setPlaneSize` re-places it) — so every generation lies behind its parent's face; `bridgeLength`/`planeAngle`/`linkCoordinates` are stored on the node and `recomputeSubtree` re-places a whole subtree from its (moved) root - after a drag, a snap, a relayout, or a re-measured link. The link measures where it sits on its plane through the `offsetParent` chain (`services/logic/link/measure.ts`; layout offsets, never transformed rects) and re-measures on plane-size, view-size and link-size changes through the equality-gated `updateLinkCoordinates` reducer (an unchanged measurement dispatches nothing). `useTreeUpdate` carries spawned children across relayouts keyed by IDENTITY (`sourceID@route`, in order for duplicates) and re-places them from the root's new location. The rendered bridge reads the node's own `bridgeLength`, so a configuration change never detaches existing bridges; it is DECORATION (`pointer-events: none`) — seen nearly edge-on, a bridge's hit box landed over its parent's links (2026-09-05). THE QUERY TRAVELS: the route a link spawns keeps its `?query#fragment` (`routeSuffix`), the IsoMatcher reads them off the raw route and `resolveViewItem` writes them into the node's `routeDivisions.plane`, which `Root` hands to the component as `plurid.plane.query` / `.fragments` / `.parameters` (the registered route's only when the request carried none); the tree's `route` and `sourceID` stay the pathname's address, so two links with different queries are two planes with one source. A PRESS ON A LINK IS THE LINK'S: the intent table returns `none` for a control, so a drag from a link never moves the camera, and the anchor is not natively draggable either (`elements.link.draggable`, default `false`) — no link ghost out of a plane.
...
- Snapshots are REFERENCES to the immutable slices (cheap); stack capped at `HISTORY_LIMIT` 100; a fresh user action clears the redo stack; `meta.remote` actions (a peer's collaboration apply) are skipped - a peer's change is not in YOUR undo - and REBASE every local snapshot (2026-09-10, `services/logic/arrangement/rebase.ts`: `arrangementEntries` turns an arrangement into per-plane entries (shown, parent, pin, hand-set size) and per-link records, `diffArrangement` names what restoring a snapshot changes, `applyArrangementDiff` replays it over the peer's arrangement through `updateTreePlaneFields` / `removePlaneFromTree` / `pruneLinks`; `rebaseSnapshot` drops a snapshot with nothing local left), so an undo after a peer's change keeps the peer's work.
...
- Kit adoption has advanced faster than adoption of the engine control/persistence/collaboration surfaces. Denote's one-tick publication deferrals are obsolete (2026-09-10): commands are accepted from `onReady` on in both modes (the router takes `onReady` / `pubsub`), and a command published earlier is reported as dropped in development.
