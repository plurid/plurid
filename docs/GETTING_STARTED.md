...
The rail at the bottom-right is the same in a space: the globe fits everything, the page pill reads the selected plane (else the one under the pointer, else the nearest) as a page — the plane fills the view, the chrome steps aside — and Escape or the cube pill brings the space back.
