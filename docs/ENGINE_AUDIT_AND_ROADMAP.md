...
**Known, pre-existing (NOT caused by this work):** a link-spawned child plane does not survive a persist→reload, even though `tree` is in the persisted fields — the tree handed to `setTree` can lose the spawned child on a layout-resolve re-attach (the rounded-location matching in `View.treeUpdate`). `reconcileTree`/`React.memo`/context memoization only swap object *references* and can never remove a node, so this is upstream of the memoization work. Transform/camera/`activePlaneID` round-trip correctly. Worth a separate fix for Spatial Notes (spawned planes ARE the notes).
