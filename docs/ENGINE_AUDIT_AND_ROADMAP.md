...
## Progress — Phase D: per-plane re-render scope CONSUMED — memoization payoff landed + verified (2026-06-20)

The structural-sharing foundation is now **fully consumed**: per-plane memoization works end-to-end. Measured at the render-test harness with a temporary render-count hook (since removed). **Results — orbit: 0 plane re-renders (was all 40/frame); hover one plane: 1 plane (was all 40); link-spawn: 48 → 15 total re-renders** (the changed branch + its new child; unchanged siblings re-render ~2× on a discrete spawn instead of 8×). Full build + engine/data/react/functions tests green; lint clean; harness verified render + spawn(bridge 160) + **real** orbit (`matrix3d`, Viewcube re-orients) + no console errors.

It took **four** pieces, not the two the earlier note predicted — found by instrumenting `PluridRoot`/`PluridPlane` prop-diffs frame by frame:
1. **Memoized per-id parent selector** — `selectors.space.makeGetTreePlaneByID()` (factory form, one memoized `createSelector` per `connect` instance) reads a `Map<planeID,node>` index (`getPlaneIndex`, rebuilt only when the tree ref changes — *not* during orbit's per-frame transform dispatches). `PluridPlane` selects `stateParentPlane` by id instead of walking the whole `getTree` (killed the O(n²)-per-frame hazard).
2. **`React.memo` on `PluridRoot` *and* `PluridPlane`** — `connect` alone does **not** gate re-renders here: `<PluridRoots>` subscribes to the per-frame transform matrix, so it re-renders constantly, and react-redux v9 `connect` re-runs the wrapped component on parent renders unless it is memoized.
3. **Reducer-level structural sharing — `reconcileTree` (`engine tree/logic.ts`).** Every `setTree` producer (link spawn, layout recompute, resize, restore) rebuilds the tree from scratch, handing even unchanged roots fresh identities. The `setTree` reducer now reconciles the incoming tree against `original(state.tree)`, reusing the previous reference for every value-equal node/array (with `width`/`height` carry-forward, since `compute()` emits 0 for those). 5 new unit tests. This is what makes `React.memo`'s shallow compare actually bail.
4. **Memoized `Context.Provider` value + `stateActivePlaneID` → derived boolean.** The decisive find: `pluridContext` was a fresh object literal every `View` render, so **every `useContext(Context)` consumer re-rendered regardless of `React.memo`** (memo can't gate context-driven renders). Wrapped it in `useMemo` (+ `registerPubSub` in `useCallback`). And `PluridPlane` now selects a per-instance `stateIsActivePlane` boolean instead of the shared `activePlaneID` string — so a hover/active change re-renders only the 2 affected planes, not all 40.

Also fixed `updateTreeByPlaneIDWithLinkCoordinates` (`general/tree/index.ts`) to be structurally shared (it rebuilt every node that merely *had* children).

## Progress — Phase D: spawned planes now survive reload + persistence hardened (2026-06-20)

**Bug:** a link-spawned child plane (i.e. a Spatial Note) did not survive a persist→reload. Root-caused with live store inspection (NOT the layout-resolve re-attach I first suspected — that's fine): the store correctly holds the spawned child (`state.space.tree` geometry `children: [detail]`, DOM = 6 planes), and `state.local.save` serializes it correctly **when it runs**. The miss is timing — persistence is a trailing **300 ms debounce**, and a full browser **reload/navigation does NOT run React's `componentWillUnmount`** (the only place a pending write was flushed). So a change made in the last debounce window before reload was simply never written. (The automated harness made this maddening to see: its tab runs `hidden`, so Chrome clamps the 300 ms timer to ≥1 s and the spawn's settling dispatches keep it from ever firing — a measurement artifact layered on the real gap.)

**Fix** (`Application/index.tsx`): flush the pending debounced write **synchronously on `pagehide` + `visibilitychange:hidden`** (covers reload, navigation, tab close, tab switch; bfcache- and mobile-safe where `beforeunload`/`unload` are not), with listener cleanup in `componentWillUnmount`. **Verified on the clean build: spawn → reload → 6 planes, detail restored, bridge 160, no errors.**

**Also hardened the persistence primitive** (`engine state/local/index.ts`): `save()` previously wrapped *both* `serialize()` and `localStorage.setItem()` in one silent `catch`. A `serialize`/`JSON.stringify` failure (a cycle / DOM / function leaking into the persisted fields) is a real code bug — it would silently disable all persistence — so it's now split out and warned **once**, while the environmental `setItem` miss (quota / private mode) stays quiet/best-effort.
...
- ~~Delete the bulkier dead code (matrix3d `*Plurid`, `printMatrix`)~~ **DONE (2026-06-20)** — removed `rotatePlurid`/`translatePlurid`/`scalePlurid` from `matrix3d/index.ts` (verified zero call sites in live engine + plurid-react; only the archived `plurid-html/prev` uses its *own* copies) and `printMatrix` from `transform/general/index.ts` (was used only as debug console-prints inside the already-`xit`-skipped transform test). ~212 LOC. Cleaned the dead imports + 28 debug-print lines from the transform test. Build + engine tests (40 pass / 37 skip, unchanged) + plurid-react build all green. **NOT deleted: the 9 quaternion fns — they are LIVE** (`mathematics/matrix/index.ts` calls `computeQuaternionFromEulers`/`quaternionMultiply`/`makeRotationMatrixFromQuaternion`). Still open: the matrix3d helper fns that only the deleted `*Plurid` used may now be dead (needs the same call-site check before removal — they're also exercised by the skipped transform test), and the ~46 commented `console.log` blocks (cosmetic).
- API surface: ~~flat config preset~~ **DONE (2026-06-20)** — `definePluridConfiguration(flat)` (engine `general/configuration`, re-exported from `@plurid/plurid-react`) + the `FlatPluridConfiguration` type (plurid-data). Flattens the ~16 most-common options (`theme`/`layout`/`center`/`planeWidth`/`bridgeLength`/`firstPerson`/`toolbar`/…) into one level, expands them to their nested locations, layers an `extend` nested-partial escape hatch on top, then runs the existing tested `merge` over defaults (so `theme` is resolved + every omitted field keeps its default). 4 unit tests; builds + lint green. Still open: unify the dual default+named export (breaking), type `StateContext` + a dev-mode "missing context" assertion.
...
### 2.3 — All planes re-render on any mutation; zero `React.memo` (HIGH) — ✅ RESOLVED (2026-06-20)
`Plane` and `Root` selected the **whole** `getTree`, so any single mutation changed tree identity → all 40 Planes + 40 Roots re-rendered. No `React.memo` anywhere.
- **Fixed** — per-id memoized parent selector + `React.memo` on Plane/Root + reducer-level `reconcileTree` structural sharing + memoized `Context.Provider` value + derived `stateIsActivePlane` boolean. **Measured: orbit 0 re-renders, hover 1 plane (was 40), spawn 48→15.** See the Phase D progress section above for the full recipe + why all four pieces were required.
...
- ~~`getTreePlaneByID(...)` called in `Plane` render body = O(n) walk × n planes = O(n²).~~ **DONE (2026-06-20)** — the walk is gone; the parent node arrives as a `stateParentPlane` prop resolved off the memoized `Map<planeID,node>` index (`makeGetTreePlaneByID`).
...
### 4.3 — Duplication (MED)
- **PARTLY DONE (Phase D).** The clearest dup is gone: `removePageFromTree` was byte-for-byte identical to `removePlaneFromTree` and had no production caller (only its own test) → deleted; its test was repointed at the live `removePlaneFromTree`, which had been **untested** (so this also added coverage to the live remove path). Engine tests green. **Deferred:** the broader "extract one `mapTree`/`walkTree`/`findInTree` for the remaining 6–7 walks" — each walk differs (toggle-show vs remove vs find vs reconcile-with-structural-sharing), so a single generic abstracts the most safety-critical engine code for marginal DRY; the explicit versions are already immutable + structurally-shared and several are individually tested. Worth a focused, separately-reviewed pass, not a tail-end change.
...
- **DONE (Phase D)** custom Redux `StateContext` was `createContext<any>({})` (`state/context/index.ts`) — a forgotten `{context: StateContext}` in `connect()` failed silently. Now typed `React.createContext<ReactReduxContextValue<any> | null>(null)`: the `null` sentinel is react-redux's own "no Provider above me" marker, so `connect`/hooks now **throw a clear error** instead of binding to a phantom `{}` store — i.e. the typing doubles as the dev assertion. Verified: no cascade across the 43 `connect({context})` sites (`pnpm check` error count unchanged), build green, harness renders 5 planes with no console errors.
...
> **DONE (Phase E, 2026-06-20) — tooling baseline.** Verified green via `pnpm install && pnpm -r build && pnpm -r test`.
> - **Dead rollup/@zerollup toolchain purged** (the HIGH item): deleted 14 `rollup*.js` config files; stripped `rollup`/`@rollup/*`/`rollup-plugin-*`/`ttypescript`/`typescript-transform-paths`/`@zerollup/*` devDeps + the dead `watch`/`start` rollup scripts from 9 package.jsons (3 rollup `build.development` → `tsup --watch`); also removed the leftover `ts-patch` `prepare` hook + devDep from `plurid-icons-react`/`plurid-ui-components-react`. Build+test green after `pnpm install` actually evicted them from `node_modules`.
> - **Shared `tsconfig.base.json`**: 12 active packages now `extends` it; `target` ES6 → **ES2020** (tsup controls the runtime target, so this is types-only). Each keeps only its path-relative `baseUrl`/`paths`/`outDir`/`include`. Build + DTS + `pnpm check` green, no new errors.
> - **Archived `plurid-canvas` + `plurid-html`**: `!` exclusion globs in `pnpm-workspace.yaml` (zero dependents/imports). This unblocked the whole build loop (html's stale `stencil build --docs` was failing it).
> - **Prettier** added (`.prettierrc.json` + `.prettierignore` + root devDep + `format`/`format.check`). **CI already existed** and is sound (`.github/workflows/ci.yml`: install → build → test → lint) — the audit's "no CI" was stale. `engines.node` unified to `>=18` across active packages.
> - **Deferred / corrected:** the full ESLint-9 flat-config migration is left out (it would flood on the ~189 `as any` etc., and tangles with the per-package legacy `.eslintrc.js`; the audit explicitly permits scoping lint out). The audit's "dead `@plurid/elementql`" is **stale** — `plurid-react`'s `ExternalPlane` genuinely imports `@plurid/elementql-client-react` (externalized at build), so it was kept.
...
|---|---|---|
| `plurid-canvas` | abandoned `0.0.0-0`, only a sanity test | **ARCHIVED (Phase E)** — excluded via `!` glob in `pnpm-workspace.yaml`; source kept on disk |
| `plurid-html` | pure Stencil, stale since 2021, fully duplicates the engine, **no `@plurid` deps** | **ARCHIVED (Phase E)** — excluded via `!` glob in `pnpm-workspace.yaml`; source kept on disk |
