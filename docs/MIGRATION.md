...
- `space.bridge.taper { run, tip }` and `space.bridge.lean { knee, limit }`: what a bridge LOOKS like and how far it follows a scrolled link. A bridge is drawn as a band where it leaves the plane (`run` px at the strip's full height, flush with the plane's top, unchanged) and tapers to a thread (`tip`) at the far end, fading as it goes: a resting bridge reads as the band it always was, a long one (a link scrolled far, a fan's later sibling at `length + i (childWidth + gap)`) stops reading as an opaque plate across the space. And it LEANS: exactly at the link up to `knee` degrees, easing toward `limit` after that and never reaching it, with the far end always landing on the parent's face - where it used to stretch to the link's own point, turning a 100px bridge into a 500px diagonal at 78 degrees. Defaults `{ run: 60, tip: 4 }` and `{ knee: 12, limit: 22 }`. `taper: { run: 0, tip: 30 }` and a `lean` limit at the knee give the flat band and the exact pointing of every release before 2026-09-18.
