...
| `space.navigation.childFraming` (new) | - | `pair` | a spawn, a link click and `space.navigateToPlane` frame the child AND its parent from the yaw between them (`−45.05°` for a 90.1° child: each at 0.706 of its width) instead of the child alone face-on, which left the parent edge-on. `childFraming: 'plane'` puts it back. |
...
- `PluridLink` takes `mode: 'toggle' | 'open'`: what a click does when the plane it opened is already open. `toggle` (the default, unchanged) puts it away; `open` goes to it, framed with its parent as `navigation.childFraming` says. Both bring a put-away plane back. A product's way back to a branch it spawned is an `open` link; the space's own links keep toggling.
