...
const computeColumnLayout = (
    ...
    const widths = groupSizes(roots, columnCount, columnOf, (root) => placedWidth(root, width), width);
    const heights = groupSizes(roots, length, rowOf, (root) => placedHeight(root, configuredHeight), fallbackHeight);
