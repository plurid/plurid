...
const App = () => {
    ...
    // ?scrollable=1 → the GEOMETRY readout is a scroller (28 filler rows in a 120px box): the wheel
    //                  over it belongs to the content, the rest of the plane still zooms
    const scrollableFlag = params.get('scrollable') === '1';
    ...
    // Links in links in links: the DETAIL plane links to three sub-planes, each of which links
    // further (mesh → edges / normals, uv → atlas, lod → levels), and the leaves link back to an
    // already-shown root (/material) — a tree of spawned planes to explore from one plane.
            ...
            component: () => (
                <>
                    ...
                    <Panel
                        ...
                        scrollable={scrollableFlag && panel.route === '/geometry'}
                    />
                </>
            ),
        })),
